...
if(isset($_POST['submit'])){
   ...
   $update_profile_name = $conn->prepare("UPDATE `cashier` SET name = ? WHERE id = ?");
   $update_profile_name->execute([$name, $cashier_id]);
   ...
   if($old_pass == $empty_pass){
      ...
      if($new_pass != $empty_pass){
         $update_admin_pass = $conn->prepare("UPDATE `cashier` SET password = ? WHERE id = ?");
         $update_admin_pass->execute([$confirm_pass, $cashier_id]);
